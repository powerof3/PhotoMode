...
namespace
{
	...
	std::filesystem::path GetTempFile(std::wstring_view a_name)
	{
		...
		if (length == 0 || length >= MAX_PATH)
			return std::filesystem::path(a_name);
...
namespace PhotoMode::IGCSBridge
{
	...
	void Bridge::OnFrameUpdate()
	{
		...
		if constexpr (kVerboseDiagnostics) {
			...
			if ((GetAsyncKeyState(VK_F4) & 0x0001) != 0) {
				if (runtimeTestActive)
					StopRuntimeDiagnostic(true);
				else
					RunExhaustiveDiagnostic();
			}

			if (runtimeTestActive)
				AdvanceRuntimeDiagnostic();
	...
	bool Bridge::CaptureCamera(CameraSnapshot& s) const
	{
		...
		if (!camera || !camera->IsInFreeCameraMode() || !camera->currentState)
			return false;
		const auto state = static_cast<RE::FreeCameraState*>(camera->currentState.get());
		if (!state)
			return false;
		s.position = state->translation;
		s.pitch = state->rotation.x;
		s.yaw = state->rotation.y;
		s.roll = MANAGER(PhotoMode)->GetViewRoll();
	...
	bool Bridge::ApplyCamera(const CameraSnapshot& s) const
	{
		...
		if (!state)
			return false;
		state->translation = s.position;
		state->rotation.x = s.pitch;
		state->rotation.y = s.yaw;
		camera->worldFOV = s.fov;
		MANAGER(PhotoMode)->SetViewRoll(s.roll);
		return true;
	}
	...
	void Bridge::DumpCameraAndConnectorState(std::string_view reason)
	{
		...
		if (cameraToolsBuffer) {
			...
			for (std::size_t off = 4; off < 256; off += 4) {
				...
				if (std::isfinite(v) && std::abs(v) > 0.0000001f)
					nonZero += std::format(" [{}]={:.6g}", off, v);
			}
	...
	void Bridge::AdvanceRuntimeDiagnostic()
	{
		...
		if (++runtimeFrameCounter < kHoldFrames)
			return;
		runtimeFrameCounter = 0;
	...
	void Bridge::TryConnectToIGCSConnector()
	{
		...
		for (DWORD i = 0; i < count; ++i) {
			...
			if (!connect && !getBuffer)
				continue;
			if (connect)
				connect();
			if (getBuffer && (cameraToolsBuffer = getBuffer())) {
				if (!connectorConnectedLogged) {
					connectorConnectedLogged = true;
					WriteStatus("Connected directly to IgcsConnector.addon64.");
				}
				return;
			}
		}
		if (!connectorSearchLogged) {
			connectorSearchLogged = true;
			WriteStatus("IgcsConnector exports not found yet; Photo Mode continues normally.");
		}
	}
	...
	void Bridge::PublishCameraData(bool available)
	{
		if (!cameraToolsBuffer)
			return;
		CameraSnapshot s{};
		if (sessionActive && currentSample.valid)
			s = currentSample;
		else if (sessionActive && sessionBase.valid)
			s = sessionBase;
		else if (!available || !CaptureCamera(s)) {
			ClearCameraData();
			return;
		}
		const Basis basis = BuildBasis(s.pitch, s.yaw, s.roll);
		std::memset(cameraToolsBuffer, 0, 512);
		cameraToolsBuffer[0] = 1;
		cameraToolsBuffer[1] = 1;
		cameraToolsBuffer[2] = sessionActive ? 1 : 0;
		WriteFloat(cameraToolsBuffer, 4, s.fov);
		WriteVec3(cameraToolsBuffer, 8, s.position);
		...
		WriteIdentityMatrix(cameraToolsBuffer, 100);
		WriteVec3(cameraToolsBuffer, 164, basis.up);
		...
		WriteFloat(cameraToolsBuffer, 200, s.pitch);
		WriteFloat(cameraToolsBuffer, 204, s.yaw);
		WriteFloat(cameraToolsBuffer, 208, s.roll);
	}
	...
	void Bridge::LoadFocusConfiguration()
	{
		...
		while (std::getline(in, line)) {
			...
			try {
				...
				if (key == "FocusScreenX")
					focusScreenX = std::clamp(value, 0.0f, 1.0f);
				else if (key == "FocusScreenY")
					focusScreenY = std::clamp(value, 0.0f, 1.0f);
				else if (key == "FocusDistance")
					focusDistance = std::max(value, 0.01f);
				else if (key == "AspectRatio")
					focusAspectRatio = std::max(value, 0.1f);
			} catch (...) {}
		}
	}
	...
	std::uint8_t Bridge::StartScreenshotSession(std::uint8_t type)
	{
		Initialize();
		CameraSnapshot s{};
		if (!photoModeActive || !CaptureCamera(s)) {
			WriteStatus("IGCS session start failed: Skyrim free camera is unavailable.");
			return 1;
		}
		sessionBase = s;
		currentSample = s;
	...
	void Bridge::EndScreenshotSession()
	{
		if (sessionBase.valid)
			if (const auto camera = RE::PlayerCamera::GetSingleton())
				camera->worldFOV = sessionBase.fov;
		AppendDiagnostic(std::format(
		...
		autoFinalPending = false;
		PublishCameraData(photoModeActive);
		WriteStatus("IGCS session ended; camera restored.");
	}

	void Bridge::MoveCameraMultishot(float lrRaw, float udRaw, float fov, bool fromStart)
	{
		if (!sessionActive || !sessionBase.valid)
			return;
		if (fromStart) {
			currentLeftRight = lrRaw;
			currentUpDown = udRaw;
		} else {
			currentLeftRight += lrRaw;
			currentUpDown += udRaw;
		...
		if (std::abs(currentLeftRight) > 0.000001f)
			++nonZeroLRCommands;
		if (std::abs(currentUpDown) > 0.000001f)
			++nonZeroUDCommands;
		if (fov > 0.0f)
			if (const auto camera = RE::PlayerCamera::GetSingleton())
				camera->worldFOV = fov;
	...
	void Bridge::DiagnosticRenderHook(RE::FreeCameraState* state, const RE::NiPoint3& original)
	{
		...
		if (sessionActive && sessionBase.valid && autoLoggedSamples < 128) {
			...
			if (serial != 0 && serial != lastLoggedMovementSerial) {
				...
				if (std::abs(autoLR) <= 0.000001f && std::abs(autoUD) <= 0.000001f)
					++zeroOffsetSamples;
				else
	...
	void Bridge::MoveCameraPanorama(float angle)
	{
		if (!sessionActive || !sessionBase.valid)
			return;
		CameraSnapshot s = sessionBase;
		s.yaw += angle * kDegToRad;
		s.valid = true;
		if (ApplyCamera(s))
			currentSample = s;
	}
	...
	{
		Initialize();
		WriteStatus(std::format("GetFreeCameraTranslation hook installed: vtable=0x{:016X}, index=0x{:X}", vtable, index));
	}
